...
import { ShieldCheck, ShieldAlert, ShieldClose, Hourglass, Mail, MessageSquare, ArrowUpRight } from 'lucide-react';
        ...
        <header className="sticky top-0 z-50 p-4">
            <nav className="container mx-auto max-w-7xl bg-black/30 backdrop-blur-sm border border-white/10 rounded-full p-2 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center space-x-6 bg-black/20 border border-white/10 rounded-full px-6 py-2">
                    ...
                    <Link to="/phishing-detector" className="text-cyan-400 font-bold transition-colors">Phishing Detector</Link>
...
// --- Main Phishing Detector Component ---
export default function PhishingDetector() {
    ...
    const handleEmailCheck = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:5000/api/analyze-email', { from_email: fromEmail, email_content: emailContent });
            ...
            <div className={`mt-8 p-6 rounded-2xl border ${currentStyle.borderColor} ${currentStyle.bgColor} transition-all duration-500`}>
                ...
                <div className="my-6">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`text-lg font-bold ${currentStyle.textColor}`}>{result.score}/100</span>
                    </div>
                    <div className="w-full bg-gray-700 rounded-full h-2.5">
                        <div className={`${currentStyle.scoreColor} h-2.5 rounded-full`} style={{ width: `${result.score}%` }}></div>
                    </div>
                </div>

                <div className="mt-6 border-t border-gray-700 pt-6">
                    <h3 className="text-lg font-semibold text-cyan-400 mb-4">Analysis Breakdown:</h3>
            ...
            <main className="container mx-auto px-6 py-12">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="mt-4 text-lg text-gray-400">
                        Paste the sender's email and the email body below. Our AI will analyze it for phishing threats.
                    </p>
                </div>

                <div className="max-w-4xl mx-auto mt-10 bg-gray-800/50 backdrop-blur-sm p-8 rounded-2xl border border-cyan-500/20 shadow-2xl">
                    <form onSubmit={handleEmailCheck} className="space-y-6">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="Sender's Email (e.g., support@example.com)"
                                required
                        ...
                        <div className="relative">
                            <MessageSquare className="absolute left-4 top-5 text-gray-500" />
                            <textarea
                                value={emailContent}
